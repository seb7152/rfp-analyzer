...
OpenRouter n'offre pas de traitement par lots asynchrone : chaque appel de complétion dure aussi longtemps que le modèle génère. Le projet est sur **Vercel Hobby avec Fluid compute** (à activer dans les réglages du projet si ce n'est pas fait ; vérifie-le et dis-le dans le résumé) : 300 s maximum par invocation, tout compris. Deux décisions en découlent.
...
9. **Fluid compute.** S'il n'est pas activé sur le projet, la route travailleur est limitée à 60 s : arrête-toi et dis-le, l'activation est un réglage de projet qui m'appartient.
10. **Catalogue indisponible.** Si `GET /api/v1/models` échoue, la fiche d'agent garde le modèle déjà enregistré et affiche l'identifiant brut avec un message ; elle n'empêche pas d'enregistrer.
