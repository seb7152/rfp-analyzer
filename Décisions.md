...
- Pages hôtes `/parametres`, `/referentiel`, `/export`, `/soutenances`, `/financial-grid`, `/tree-view`, `/documents` : composants d'origine sous le nouveau shell et les nouveaux tokens. À reprendre pour terminer l'harmonisation : cartes KPI de `WeightsTab`, tableaux `SuppliersTab` / `AnalystsTab` / `VersionsTab`, les modales d'import DOCX et de dépôt de documents, `RFPSwitcher` / `VersionSwitcher` / `OrganizationSwitcher` (fonctionnels, style intermédiaire), la page de connexion (bouton indigo), `ThreadPanel`, `PDFViewerSheet`.
...
## D-18 — Atelier d'import : la lecture avant l'écriture

- **Décision.** L'assistant par étapes (`ImportWithStepper`, quatre étapes imposées, JSON collé, validation réduite à `JSON.parse`) est remplacé par un atelier à deux volets, à l'adresse `/import` (l'ancienne `/import/json` y redirige, en conservant `?supplier=`). À gauche, les quatre jeux de données avec ce qu'ils contiennent déjà — domaines, exigences, fournisseurs, puis réponses éclatées par fournisseur — dans l'ordre que l'on veut. À droite, la lecture du fichier déposé ou du JSON collé.
  - *Aperçu avant écriture.* Chaque ligne est marquée : à créer, à mettre à jour, ignorée, ou en erreur avec la raison en français (« le domaine « Astreinte » n'existe pas », « level doit être un entier de 1 à 4 »). Le pied totalise, le bouton annonce ce qu'il va faire (« Importer 78 lignes »).
  - *Seules les lignes valides partent.* Les routes d'import valident le payload en bloc : une ligne fautive et rien ne passe. L'atelier applique les mêmes règles côté client et n'envoie que ce dont il répond ; les lignes refusées restent à l'écran avec leur motif. Les règles sont donc écrites à deux endroits : proposition P-9 dans REFONTE.md pour un rapport ligne à ligne côté serveur.
  - *Sémantique respectée.* Domaines, exigences et réponses sont en upsert : un code déjà présent est mis à jour, et l'aperçu le dit. Les fournisseurs sont en insert simple : un identifiant déjà déclaré est marqué « déjà déclaré, ignoré » et retiré de l'envoi, sinon l'import entier échouerait.
  - *Le format attendu est un prompt, et il y en a deux.* « Format attendu » ouvre quatre onglets. *Prompt · fichier* : à coller dans un assistant avec un tableur, il produit le JSON à déposer ici. *Prompt · agent* : pour un assistant branché sur le connecteur MCP, il donne l'adresse du serveur, l'en-tête d'authentification, la consigne de demander son jeton à l'utilisateur s'il ne l'a pas et de ne jamais l'écrire, le `rfp_id` de la consultation, l'outil à appeler (`import_structure`, `import_requirements`, `import_supplier_responses`) et le détour par `get_import_command` quand un script a déjà écrit le fichier sur disque. Les deux portent les champs, les règles et le vocabulaire de la consultation : domaines existants avec leurs codes, identifiants de fournisseurs déjà pris, codes d'exigences, fournisseur visé. Puis le JSON Schema et la liste des champs. Le prompt demande une conversion déterministe — correspondance de colonnes établie une fois puis appliquée à toutes les lignes, script écrit et exécuté dès que le tableau est long ou irrégulier, textes repris tels quels — parce qu'une transcription ligne à ligne dérive. Chaque bloc se copie par une icône discrète dans son coin, sans bouton plein. C'est la réponse à l'absence d'import de tableur : la conversion se fait à l'extérieur, mais avec tout ce qu'il faut pour tomber juste.
- **Parcours / persona.** Sophie (friction 2) ; le dépôt des réponses fournisseur par fournisseur sans quitter l'écran.
- **Options écartées.** Garder le stepper en le repeignant (l'ordre imposé était la friction, pas la couleur) ; proposition 1 « Plan d'import » en panneaux empilés, écartée par l'utilisateur au profit de l'atelier ; un import XLSX côté application (développement hors périmètre, et le prompt couvre le besoin sans nouvelle dépendance) ; envoyer le fichier entier et afficher l'erreur du serveur (un fichier de 200 exigences se serait bloqué sur une ligne).
- **Caduque si.** Les routes d'import renvoient un rapport ligne à ligne (P-9) : la validation client redevient un simple confort d'affichage.
