...
- **Décision.** L'assistant par étapes (`ImportWithStepper`, quatre étapes imposées, JSON collé, validation réduite à `JSON.parse`) est remplacé par un atelier à deux volets, à l'adresse `/import` (l'ancienne `/import/json` y redirige, en conservant `?supplier=`). À gauche, les quatre jeux de données avec ce qu'ils contiennent déjà — domaines, exigences, fournisseurs, puis réponses éclatées par fournisseur — dans l'ordre que l'on veut. À droite, la lecture du fichier déposé ou du JSON collé.
  ...
  - *Le format attendu est un prompt.* « Format attendu » ouvre le schéma sous trois formes : un prompt prêt à coller dans un assistant avec un tableur, qui porte les champs, les règles et le vocabulaire de la consultation (domaines existants avec leurs codes, identifiants de fournisseurs déjà pris, codes d'exigences, fournisseur visé) ; le JSON Schema ; la liste des champs. Le prompt demande une conversion déterministe — correspondance de colonnes établie une fois puis appliquée à toutes les lignes, script écrit et exécuté dès que le tableau est long ou irrégulier, textes repris tels quels — parce qu'une transcription ligne à ligne dérive. Chaque bloc se copie par une icône discrète dans son coin, sans bouton plein. C'est la réponse à l'absence d'import de tableur : la conversion se fait à l'extérieur, mais avec tout ce qu'il faut pour tomber juste.
