  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case "strong": {
        ...
        runs.push(
          ...inlineTokensToRuns(t.tokens ?? [], { ...ctx, bold: true })
        );
        break;
      }
      case "em": {
        ...
        runs.push(
          ...inlineTokensToRuns(t.tokens ?? [], { ...ctx, italics: true })
        );
        break;
      }
      case "del": {
        ...
        runs.push(
          ...inlineTokensToRuns(t.tokens ?? [], { ...ctx, strike: true })
        );
        break;
      }
      case "codespan": {
        ...
        runs.push(
          new TextRun({ text: sanitize(t.text), font: "Courier New", size: 18 })
        );
        break;
      }
      case "link": {
...
function splitCellContent(
  raw: string
): { isBullet: boolean; content: string }[] {
...
function cellToChildren(cell: {
  ...
  if (items.length <= 1) {
    ...
    return [
      new Paragraph({ children: blockToRuns(cell), spacing: { after: 0 } }),
    ];
  }
