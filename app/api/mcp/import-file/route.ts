 ...
 *
 * Auth: short-lived import token generated by get_import_command only.
 *   Regular PATs are not accepted here.
 *
...
/**
 * Resolve auth context from a short-lived import token only.
 */
function resolveAuth(req: NextRequest): MCPAuthContext | null {
...
export async function POST(req: NextRequest) {
  // ── Auth ──────────────────────────────────────────────────────────────────
  const authContext = resolveAuth(req);
  if (!authContext) {
    return json({ error: "Unauthorized: valid import token required (use get_import_command to generate one)" }, 401);
  }
