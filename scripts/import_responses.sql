-- =====================================================
-- RFP Responses Import Script
-- Generated: 2025-11-15T23:29:13.757Z
-- Total responses: 126 (18 requirements × 7 suppliers)
-- =====================================================

DO $$
DECLARE
    v_rfp_id UUID := 'YOUR_RFP_ID_HERE'; -- Replace with actual RFP ID
    v_supplier_id UUID;
...
BEGIN
    ...
    -- Get supplier ID for Accenture
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'Accenture' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "Accenture" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing Accenture (ID: %)', v_supplier_id;

        -- Response 1/18: R - 1
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 1' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 1" not found - skipping';
...
No reference to existing tools/automation (e.g., RPA) takeover for monitoring activities.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 1: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 2' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 2" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No samples/templates (SOP/KB taxonomy) and no commitment to translation during the transitional phase.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 2: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 3' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 3" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Single primary site (Mauritius) raises resilience risk without an EMEA backup.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 3: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 4' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 4" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
The move to full offshore in Year 2 could dilute guaranteed French L1.5 coverage if not explicitly preserved.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 4: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 5' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 5" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Assumptions place Level 0 filtering on Accor before assignment to Accenture; Accor’s target workflow expects provider-led triage/dispatcher within L1.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 5: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 6' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 6" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No explicit shared-resource model details (cross‑trained pool, language mix',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 6: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 7' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 7" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 7: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 11' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 11" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No report delivery SLAs (e.g., monthly), no KPI dictionary (formulas, business-hours clocks, pause rules)',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 11: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 8' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 8" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Planned shift of SDM from Paris (Year 1) to Mauritius (Year 2) may reduce proximity to Accor’s user community unless mitigated.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 8: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 9' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 9" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Escalation SLAs and contact matrix: No explicit response-time targets for escalations or a named deputy/backfill plan for AM/SDM absences.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 9: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 10' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 10" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Escalation and communication mechanisms are described via governance principles (transparency, agile adjustments, feedback loops) and the dual onshore/offshore SDM model in Year 1 for proximity and responsiveness.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 10: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 13' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 13" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No quantified targets (e.g., recurrence rate reduction, FCR uplift, MTTR reduction) or timelines to implement corrective actions post-RCA.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 13: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 14' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 14" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No time‑bound AI roadmap (pilots, scale, security/guardrails).',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 14: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 18' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 18" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Resource plan is referenced as a deliverable but not shown in the proposal (no FTE by role/site/language for the transition period).',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 18: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 19' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 19" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No sample Risk Register/heat map included (owners, impact/likelihood scoring, triggers).',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 19: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 20' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 20" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
No explicit  inventory-based handover checklist, or access revocation plan.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 20: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 21' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 21" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Quarterly upfront billing and COLA escalators may weaken cost optimization if activity drops.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 21: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
        ...
        BEGIN
            SELECT id INTO v_requirement_id
            FROM requirements
            WHERE reference = 'R - 22' AND rfp_id = v_rfp_id;

            IF v_requirement_id IS NULL THEN
                RAISE WARNING '  Requirement "R - 22" not found - skipping';
                v_error_count := v_error_count + 1;
            ELSE
                INSERT INTO responses (
                    requirement_id,
...
Some gains are contingent on deploying Accenture tools (e.g., GenWizard); if not approved, Accenture reserves the right to reassess productivity and financial commitments.',
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '  Error inserting response for R - 22: %', SQLERRM;
                v_error_count := v_error_count + 1;
        END;
    ...
    -- Get supplier ID for ITCI
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'ITCI' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "ITCI" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing ITCI (ID: %)', v_supplier_id;
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Explicit scope coverage: ITC states L1 and L1.5 support are in scope and includes Monitoring, Administrative, and Closing activities with centralized ticket logging and tracking/closure of issues. ITC also agrees with the ticket triage and routing role.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'ITC recognizes both Option 1 (08:00–19:00 CET) and Option 2 (08:00–21:00 CET) , and explicitly proposes running with Option 2 for “wider coverage” with “no impact to resource model.”
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Explicit alignment to Accor’s two scenarios: States Scenario 1 “L1 Support – English, L1.5 Support – French” and Scenario 2 “L1 & L1.5 Support – English.” 
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Hybrid footprint with onsite France for governance and offshore India for scale. Clear roles (on-site SDM FR/EN for Y1, then moves offeshire; on-site Oracle Finance Lead FR/EN), L1/L1.5 team, separate Admin/Closing/Monitoring stream, and Technical/Automation stream. SLA/KPI-driven governance, TMO, and detailed transition approach are provided.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'ITC sets a “Baseline Benchmarking” rule where any change beyond ±10% in scope (operating units/~70 users) “triggers workload impact analysis and commercial adjustment.” with is not reliable
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Demonstrates Oracle EBS R12 expertise via case studies  with quantified outcomes (SLA ≥95%, 25–40% ticket reduction, 30% cost savings). Proposed team CVs show 10–14+ years on EBS R12 across Finance/SCM; clear split between functional and technical streams.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Retention approach: Mentions continuous improvement, cross-skilling, embedded SME champions, and career/learning programs to boost tenure and quality. Risk register addresses attrition with mitigation (incentives, backups, accelerated shadow/on-the-job training).
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'The proposal names both key roles: an on‑site Service Delivery Manager and an Account Manager. An on‑site Oracle Finance Lead (French/English, 1 year) supports functional oversight.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'ITC defines all required forums with purpose, participants, and frequency
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Proposes a Next‑Gen AI platform for observability, knowledge management, GenAI‑assisted resolution, and auto‑triage; embeds an Automation Factory and RRS (Runner/Repeater/Stranger) framework to target repetitive L1/L1.5 work.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'A dedicated risk register with probability/impact heat map is presented, plus concrete risks and mitigations. Mitigations include incentives/backup resources, accelerated shadowing/on‑the‑job training, creation of missing docs, early onboarding/access planning, and observability to detect anomalies early.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'The proposal includes a “Reversibility Methodology” slide but provides no substantive detail on phases, activities, roles, deliverables, acceptance criteria, or governance cadence. It does not describe knowledge transfer scope, reverse shadowing, service continuity, or handover acceptance.
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    'Clear fixed-price model in EUR with stepped reductions Y1→Y3, showing intent to pass productivity gains. Transition priced and discounted (50%).
        ...
        BEGIN
            ...
            ELSE
                INSERT INTO responses (
                    ...
                    v_supplier_id,
                    NULL,
                    ' ITC presents a clear CSI/automation program (RRS framework, Next‑Gen AI platform, SOP/KEDB, auto‑triage) with a time‑phased roadmap and quantified manual‑effort reduction targets (to be checked, may lack realism)
...
',
    ...
    -- Get supplier ID for TCS
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'TCS' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "TCS" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing TCS (ID: %)', v_supplier_id;
...
Some automation benefits depend on licensing (e.g., ServiceNow Now Assist Pro Plus) and business-case approvals, which may delay impact.',
    ...
    -- Get supplier ID for Cap Gémini
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'Cap Gémini' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "Cap Gémini" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing Cap Gémini (ID: %)', v_supplier_id;
...
COLA/indexation rules not stated; could offset step‑downs.',
    ...
    -- Get supplier ID for LUCEM
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'LUCEM' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "LUCEM" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing LUCEM (ID: %)', v_supplier_id;
...
Limited articulation of joint analysis of innovation cost consequences and how those will be priced; no sample model or precedent.',
    ...
    -- Get supplier ID for Attaway
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'Attaway' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "Attaway" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing Attaway (ID: %)', v_supplier_id;
...
Evidence baseline: No starting baseline and target ranges (e.g., % MTTR decrease, % ticket deflection) tied to committed cost adjustments.',
    ...
    -- Get supplier ID for Prérequis
    SELECT id INTO v_supplier_id
    FROM suppliers
    WHERE name = 'Prérequis' AND rfp_id = v_rfp_id;

    IF v_supplier_id IS NULL THEN
        RAISE WARNING 'Supplier "Prérequis" not found for this RFP - skipping';
    ELSE
        RAISE NOTICE 'Processing Prérequis (ID: %)', v_supplier_id;
...
',
    ...
    RAISE NOTICE 'Successfully imported: % responses', v_response_count;
    RAISE NOTICE 'Errors encountered: %', v_error_count;
    RAISE NOTICE '================================================';

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Fatal error during import: %', SQLERRM;
END $$;
