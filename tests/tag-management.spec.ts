...
test.describe('Tag Management System', () => {
  ...
  test.describe('Part 1: Basic Tag Creation', () => {
    ...
    test('TC-TAG-001: Create tag via UI', async ({ page }) => {
      ...
      const blueColorButton = page.locator('button').filter({ has: page.locator(`[style*="${TEST_TAG.color}"]`) }).first();
      await blueColorButton.click();

      // Click create button (+ icon button)
      const createButton = page.locator('button').filter({ hasText: '+' }).last();
      ...
      // Take screenshot
      await page.screenshot({ path: 'tests/screenshots/tag-created.png', fullPage: true });
    });
    ...
    test('TC-TAG-002: Tag persists after reload', async ({ page }) => {
      ...
      // Take screenshot
      await page.screenshot({ path: 'tests/screenshots/tag-persisted.png', fullPage: true });
    });
  });
  ...
  test.describe('Part 2: Tag Assignment to Requirements', () => {
    ...
    test('TC-TAG-004: Assign tag to single requirement', async ({ page }) => {
      ...
      const addTagButton = row.locator('button').filter({ hasText: '+' }).last();
      ...
      const testTagCheckbox = page.locator(`label:has-text("${TEST_TAG.name}") input[type="checkbox"]`);
      ...
      await expect(page.locator('text=Tags saved successfully')).toBeVisible({ timeout: 5000 });

      // Take screenshot
      await page.screenshot({ path: 'tests/screenshots/tag-assigned.png', fullPage: true });
    });

    test('TC-TAG-005: Assign multiple tags to requirement', async ({ page }) => {
      // Navigate to requirements view
      await page.click(`text=Support L1 Finance - Accor`);
      await page.waitForLoadState('networkidle');
      ...
      await page.locator(`label:has-text("${API_TEST_TAG.name}") input`).check();
      ...
      // Take screenshot
      await page.screenshot({ path: 'tests/screenshots/multiple-tags-assigned.png', fullPage: true });
    });
    ...
    test('TC-TAG-007: Remove tag from requirement', async ({ page }) => {
      ...
      // Take screenshot
      await page.screenshot({ path: 'tests/screenshots/tag-removed.png', fullPage: true });
    });
  });

  test.describe('Part 3: Cascade Tag Assignment', () => {

    test('TC-TAG-008: Cascade assign tags to category children', async ({ page }) => {
      // Navigate to requirements view
      await page.click(`text=Support L1 Finance - Accor`);
      await page.waitForLoadState('networkidle');
      ...
      const categoryRow = page.locator('tr').filter({ has: page.locator('text=/^[0-9]\\s+-\\s+/') }).first();

      // Look for + button on category row
      const cascadeButton = categoryRow.locator('button').filter({ hasText: '+' }).first();
      ...
      // Take screenshots
      await page.screenshot({ path: 'tests/screenshots/cascade-assigned.png', fullPage: true });
    });

    test('TC-TAG-009: Verify cascade scope (requirements only, not categories)', async ({ page }) => {
      ...
      await page.screenshot({ path: 'tests/screenshots/cascade-scope-verification.png', fullPage: true });
    });
  });
  ...
  test.describe('Part 5: UI/UX Validation', () => {
    ...
    test('TC-TAG-015: Tag manager collapse/expand', async ({ page }) => {
      ...
      // Take screenshot
      await page.screenshot({ path: 'tests/screenshots/tag-manager-collapsed.png', fullPage: true });
    });
  });
});
...
test.describe('API Tests', () => {
  ...
  test('TC-TAG-006: Assign tag via API', async ({ request }) => {
    // First get tags
    const tagsResponse = await request.get(`${BASE_URL}/api/rfps/${RFP_ID}/tags`);
  ...
  test('TC-TAG-013: Assign non-existent tag', async ({ request }) => {
    ...
    console.log('Non-existent tag response:', response.status(), await response.json());
  });
});
