...
/**
 * Model selector fed by the OpenRouter catalogue: search by name or
 * provider, context length, prices per million tokens, structured outputs.
...
export function ModelPicker({
  ...
  return (
    <div className="space-y-1.5">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            className="flex h-9 w-full items-center gap-2 rounded-md border border-input bg-background px-3 text-left text-sm transition-colors duration-150 hover:border-ring focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-60"
          >
            <span className="min-w-0 flex-1 truncate">
              {current ? current.name : value || "Choisir un modèle"}
              <span className="num ml-2 text-xs text-muted-foreground">{value}</span>
            </span>
        ...
        <PopoverContent align="start" className="w-[min(560px,90vw)] p-0">
          <Command>
            ...
            <div className="flex items-center gap-2 border-b border-border px-3 py-1.5 text-xs text-muted-foreground">
              ...
              <span className="ml-auto num">{visible.length} modèles</span>
            </div>
                ...
                {visible.map((m) => (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="num block truncate text-xs text-muted-foreground">{m.id}</span>
                    </span>
                    <span className="num shrink-0 text-right text-xs text-muted-foreground">
                      <span className={cn("block", m.context_length < MIN_CONTEXT && "text-status-partial")}>
      ...
      {error && <p className="text-xs text-status-partial">{error} Le modèle enregistré est conservé.</p>}
      ...
      {current && !current.structured_outputs && (
        <p className="text-xs text-muted-foreground">Ce modèle n&apos;annonce pas les sorties structurées : le format sera décrit dans le préambule et la sortie extraite du texte.</p>
      )}
    </div>
  );
}
