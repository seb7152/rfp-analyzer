    ...
    const versionTotals: VersionTotals[] = useMemo(() => {
        return sortedVersions.map(version => {
            ...
            return {
                ...
                versionName: version.version_name || `Version ${sortedVersions.indexOf(version) + 1}`,
                                    ...
                                    <div className="p-3 space-y-1">
                                        ...
                                        {version.version_date && (
                                            <div className="text-xs text-slate-500">
                                                {new Date(version.version_date).toLocaleDateString('fr-FR')}
                                            </div>
                                        )}
                                        {idx === 0 && (
                                            <Badge variant="outline" className="mt-1 text-[10px] bg-slate-100">
                                                Référence
                                            </Badge>
                                        )}
                                    </div>
                    ...
                    <TableBody>
                        ...
                        <TableRow className="bg-slate-100/80 font-bold hover:bg-slate-100/80 border-t-2 border-slate-200">
                            ...
                            {versionTotals.map((totals, idx) => {
                                ...
                                return (
                                    <TableCell key={totals.versionId} className="border-l p-4 min-w-[200px]">
...
function TotalRow({ label, value, previousValue, showVariation, isTco, className }: TotalRowProps) {
    const variation = showVariation && previousValue !== undefined && previousValue !== 0
        ...
        const row = (
            <TableRow
                ...
                className={cn(
                    ...
                    hasChildren && "bg-slate-50/30 font-semibold text-slate-900 border-l-2 border-l-slate-200"
                )}
            >
                ...
                {versions.map((version, versionIdx) => {
                    ...
                    const prevValue = prevVersion ? valuesMap.get(prevVersion.id)?.get(line.id) : null;

                    // Calculate total if parent
                    const calculatedTotal = hasChildren
                        ? calculateTotal(line as LineWithValues, version.id, line.line_type === 'setup' ? 'setup' : 'recurrent')
                        : null;

                    const prevCalculatedTotal = hasChildren && prevVersion
                        ? calculateTotal(line as LineWithValues, prevVersion.id, line.line_type === 'setup' ? 'setup' : 'recurrent')
                        : null;

                    // Get current and previous values for leaf nodes
                    const currentValue = hasChildren
                        ? calculatedTotal
                        : (line.line_type === 'setup' ? (value?.setup_cost ?? null) : (value?.recurrent_cost ?? null));

                    const previousValue = hasChildren
                        ? prevCalculatedTotal
                        : (line.line_type === 'setup' ? (prevValue?.setup_cost ?? null) : (prevValue?.recurrent_cost ?? null));

                    // Calculate variation
                    const variation = versionIdx > 0 && previousValue !== null && previousValue !== 0 && currentValue !== null
