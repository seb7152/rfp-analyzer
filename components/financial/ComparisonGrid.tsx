...
export function ComparisonGrid({
    ...
    // Modals
    const [createVersionModal, setCreateVersionModal] = useState<{ isOpen: boolean; supplierId: string, supplierName: string }>({
        isOpen: false, supplierId: "", supplierName: ""
    });
    const [saveModalOpen, setSaveModalOpen] = useState(false);

    // Mutations
    const { mutate: batchUpdate, isLoading: isSaving } = useBatchUpdateFinancialValues();
        ...
        if (versions.length > 0) {
            ...
            suppliers.forEach(s => {
                if (!newSelection.has(s.id)) {
                    const supplierVersions = versions.filter(v => v.supplier_id === s.id);
    ...
    const currentValuesMap = useMemo(() => {
        ...
        if (editingVersionId && editState) {
            const editingMap = map.get(editingVersionId) || new Map<string, FinancialOfferValue>();
            editState.values.forEach((val, lineId) => {
                ...
                const merged: FinancialOfferValue = {
                    ...
                    setup_cost: val.setup_cost !== undefined ? val.setup_cost : (existing?.setup_cost ?? null),
                    recurrent_cost: val.recurrent_cost !== undefined ? val.recurrent_cost : (existing?.recurrent_cost ?? null),
                    quantity: val.quantity !== undefined ? val.quantity : (existing?.quantity ?? 1),
    ...
    const handleCancelEdit = () => {
        if (editState && editState.values.size > 0) {
            if (!confirm("Vous avez des modifications non sauvegardées. Voulez-vous vraiment annuler ?")) return;
        }
        setEditingVersionId(null);
        setEditState(null);
    };

    const handleCellValueChange = (lineId: string, field: 'setup_cost' | 'recurrent_cost', value: number | null) => {
    ...
    const onSaveConfirm = () => {
        ...
        const valuesToSave = Array.from(editState.values.entries()).map(([lineId, val]) => ({
            template_line_id: lineId,
            ...val
        }));

        batchUpdate({ versionId: editingVersionId!, values: valuesToSave }, {
            onSuccess: () => {
    ...
    const calculateLineTotal = (
        ...
        if (!line.children || line.children.length === 0) {
            ...
            if (!val) return null;
            return type === 'setup' ? (val.setup_cost ?? 0) : (val.recurrent_cost ?? 0);
        }
    ...
    return (
        <div className="relative w-full h-full flex flex-col">
                            ...
                            {activeVersions.map(({ supplier, version }) => (
                                <TableHead key={supplier.id} className="min-w-[180px] p-0 align-top border-l bg-gray-50/30">
                                    <SupplierColumnHeader
                                        ...
                                        versions={versions.filter(v => v.supplier_id === supplier.id)}
                                        selectedVersionId={version?.id || ""}
                                        onVersionChange={(vid) => handleVersionChange(supplier.id, vid)}
                                        onNewVersion={() => setCreateVersionModal({ isOpen: true, supplierId: supplier.id, supplierName: supplier.name })}
                                        onImportVersion={() => { /* TODO US-7 */ }}
                ...
                <div className="sticky bottom-0 left-0 right-0 bg-white border-t p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] flex items-center justify-between z-20 animate-in slide-in-from-bottom-5">
                    <div className="flex items-center gap-4">
                        <div className="flex gap-2 text-sm text-gray-600">
                            <Button variant="outline" size="icon" onClick={handleUndo} disabled={!editState || editState.historyIndex < 0}>
                    ...
                    <div className="flex items-center gap-2">
                        <Button variant="ghost" onClick={handleCancelEdit}>Annuler</Button>
                        <Button onClick={() => setSaveModalOpen(true)} className="bg-green-600 hover:bg-green-700 text-white">
            ...
            <CreateVersionModal
                ...
                onClose={() => setCreateVersionModal(prev => ({ ...prev, isOpen: false }))}
                rfpId={rfpId}
            ...
            <SaveVersionModal
                ...
                isLoading={isSaving}
                defaultName={versions.find(v => v.id === editingVersionId)?.version_name || ""}
            />
        </div>
    );
}
            ...
            <TableRow key={line.id} className={cn("hover:bg-slate-50/50 transition-colors", hasChildren && "bg-slate-50/30 font-semibold text-slate-900 border-l-2 border-l-slate-200")}>
                ...
                {activeVersions.map(({ supplier, version }) => {
                    ...
                    const calculatedTotal = hasChildren
                        ? calculateTotal(line, valuesMap, version?.id, line.line_type === 'setup' ? 'setup' : 'recurrent')
                        : null;
