...
export function SupplierColumnHeader({
    ...
    if (isEditing) {
        return (
            <div className={cn("flex flex-col gap-2 p-2 border-b border-orange-400 bg-orange-50")}>
                <div className="flex justify-between items-center">
                    <span className="font-semibold text-sm truncate" title={supplierName}>{supplierName}</span>
                    <span className="text-xs font-medium bg-yellow-200 text-yellow-800 px-1.5 py-0.5 rounded">En édition</span>
                </div>
    ...
    return (
        <div className="flex flex-col gap-2 p-2 min-w-[180px]">
            <div className="font-semibold text-sm truncate" title={supplierName}>{supplierName}</div>
            <div className="flex gap-1 items-center">
                <Select value={selectedVersionId} onValueChange={onVersionChange} disabled={versions.length === 0}>
                    <SelectTrigger className="h-8 text-xs flex-1">
                        <SelectValue placeholder={versions.length === 0 ? "Aucune version" : "Sélectionner..."} />
                    </SelectTrigger>
                    <SelectContent>
                        {versions.map(v => (
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={onEditManual} disabled={versions.length === 0 || !selectedVersionId}>
