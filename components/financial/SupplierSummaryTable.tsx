...
function VariationCell({ variation }: { variation: number | null }) {
    ...
    return (
        <span className={cn(
            ...
            <span>{isIncrease ? '+' : '-'}{formattedVariation}%</span>
        </span>
    );
}

export function SupplierSummaryTable({ data, tcoPeriod, supplierName }: SupplierSummaryTableProps) {
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {data.map((item, idx) => {
                            ...
                            const tcoVariation = prevItem ? calculateVariation(item.tco, prevItem.tco) : null;
                            const setupVariation = prevItem ? calculateVariation(item.totalSetup, prevItem.totalSetup) : null;
                            const recurrentVariation = prevItem ? calculateVariation(item.totalRecurrentAnnual, prevItem.totalRecurrentAnnual) : null;
